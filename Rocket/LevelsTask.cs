...
namespace func_rocket
{
    ...
    {
        static readonly Physics physic = new Physics();
        ...
        {
            return new Level(name, rocket, target, gravity, physic);
        }
        ...
        public static IEnumerable<Level> CreateLevels()
        {
            ...
            yield return new Level("Up", rocket, upTarget, (size, v) => 
            new Vector(0.0, -300 / (size.Height - v.Y + 300.0)), physic);

            yield return CreateLevel("WhiteHole", (size, v) => WhiteHole(v));

            yield return CreateLevel("BlackHole", (size, v) => BlackeHole(v));

            yield return CreateLevel("BlackAndWhite", (size, v) => (WhiteHole(v) + BlackeHole(v)) / 2);
        }
        ...
        {
            return Math.Sqrt(Math.Pow(target.X - v.X, 2) + Math.Pow(target.Y - v.Y, 2)) /
                (Math.Abs((Math.Pow(target.X - v.X, 2) + Math.Pow(target.Y - v.Y, 2))) + 1);
        }
    }
}